...
void main() {
  test('git JSON binds every handler to the five declaration IDs', () async {
    ...
    expect(ids.toSet(), {
      gitGenerateSpecHandler,
      gitConfigPostProcessHandler,
      gitAliasHandler,
      gitCommitMessageHandler,
      gitConfigSuggestionsHandler,
    });
  });
  ...
  test('commit message custom handler is empty without a staged diff', () async {
    final registry = JsonHandlerRegistry();
    registerGitHandlers(registry);
    final adapter = _FakeAdapter({
      'fig settings --format json autocomplete.ai.enabled':
  ...
  test('config postProcess keeps prefixes and drops configSuggestions names',
    ...
    final suggestions = handler(
        ...
        'user.name Alice');
    expect(
        suggestions.map((item) => item.nameSingle ?? ''),
