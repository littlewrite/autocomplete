...
class _TrackedFile {
  ...
  final String file;
  _TrackedFile({required this.working, required this.file});
}

class PostProcessBranchesOptions {
  const PostProcessBranchesOptions({this.insertWithoutRemotes = false});
  final bool insertWithoutRemotes;
}
...
final List<Option> addOptions = [
  ...
  Option(
      ...
      args: [
        Arg(suggestions: [FigSuggestion(name: '+x'), FigSuggestion(name: '-x')])
      ]),
  Option(
...
final List<FigSuggestion> configSuggestions = [
  FigSuggestion(
  ...
  FigSuggestion(
      name: 'color.advice.hint', description: 'Use customized color for hints'),
  FigSuggestion(
  ...
  FigSuggestion(name: 'diff.algorithm', description: 'Choose a diff algorithm'),
  FigSuggestion(
...
final headSuggestions = [
  FigSuggestion(
      name: 'HEAD', icon: '🔻', description: 'The most recent commit'),
  FigSuggestion(
      name: 'HEAD~<N>',
...
/// Git finds these as "git-<name>" on PATH. Merge with name when building subcommands.
